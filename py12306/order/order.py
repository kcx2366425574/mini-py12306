...
from py12306.helpers.api import (
    API_INITDC_URL, API_SUBMIT_ORDER_REQUEST, API_CHECK_ORDER_INFO, API_GET_QUEUE_COUNT,
    API_CONFIRM_SINGLE_FOR_QUEUE, API_QUERY_ORDER_WAIT_TIME
)
from py12306.helpers.func import singleton, stay_second
...
class Browser:
    ...
    async def __request_init_slide(self, session, html):
        ...
        async def on_request(req):
            if req.url.startswith(API_INITDC_URL) and req.isNavigationRequest():
        ...
        while self.current_queue_wait:
            ...
            if result.get('status') and 'data' in result:
                ...
                if 'waitTime' in result_data:
                    ...
                    if wait_time == -1:  # 成功
                        ...
                        OrderLog.add_quick_log(
                            OrderLog.MESSAGE_QUERY_ORDER_WAIT_TIME_WAITING.format(
                                result_data.get('waitCount', 0), wait_time)
                        ).flush()
                    else:
                        if wait_time in [-2, -3]:  # -2 失败 -3 订单已撤销
                            OrderLog.add_quick_log(
                    ...
                    # 失败 对不起，由于您取消次数过多，今日将不能继续受理您的订票请求。XX月XX日您可继续使用订票功能。
                    # TODO 需要增加判断 直接结束
                    OrderLog.add_quick_log(
                        OrderLog.MESSAGE_QUERY_ORDER_WAIT_TIME_FAIL.format(
