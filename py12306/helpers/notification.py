...
from py12306.helpers.api import API_NOTIFICATION_BY_VOICE_CODE, API_NOTIFICATION_BY_VOICE_CODE_DINGXIN
...
class Notification:
    ...
    def voice_code(cls, phone, name, content):
        ...
        if Config().NOTIFICATION_VOICE_CODE_TYPE == 'dingxin':
            self.send_voice_code_of_dingxin(phone, name, content)
        else:
            self.send_voice_code_of_yiyuan(phone, name, content)
    ...
    def send_voice_code_of_dingxin(self, phone, name, info=None):
    ...
    def send_email_by_smtp_with_qrcode(self, to, title, qrcode_path):
        ...
        """
        html_apart = MIMEText(html_file, 'html')
        image_file = qrcode_path
        image_apart = MIMEImage(open(image_file, 'rb').read(), image_file.split('.')[-1])
        image_apart.add_header('Content-ID', '<0>')
        message.attach(image_apart)
        message.attach(html_apart)
        try:
            server = smtplib.SMTP_SSL(Config().EMAIL_SERVER_HOST)
            server.ehlo()
            server.login(Config().EMAIL_SERVER_USER, Config().EMAIL_SERVER_PASSWORD)
            server.send_message(message)
            server.quit()
